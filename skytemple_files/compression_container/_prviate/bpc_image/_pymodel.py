...
class BpcImageCompressionContainer(CommonAt):
    ...
    def decompress(self) -> bytes:
        ...
        data, len_read = FileType.BPC_IMAGE.decompress(self.compressed_data, self.length_decompressed)
        assert len_read == len(self.compressed_data)
        return data

    def to_bytes(self) -> bytes:
        return b"BPCIMG" + self.length_decompressed.to_bytes(2, "little") + self.compressed_data  # pylint: disable=no-member
    ...
    @classmethod
    def compress(cls, data: bytes) -> BpcImageCompressionContainer:
        from skytemple_files.common.types.file_types import FileType
