    ...
    def apply(self, apply: Callable[[], None], rom: NintendoDSRom, config: Pmd2Data) -> None:
        ...
        if not self.is_applied(rom, config):
            ...
            for filename in get_files_from_rom_with_extension(rom, "str"):
                ...
                sorted_list = list(
                    enumerate(strings.strings[new_pkmn_str_region - 1 : new_pkmn_str_region - 1 + NUM_NEW_ENTRIES])
                )
                sorted_list.sort(key=lambda x: normalize_string(x[1]))
                sorted_list2 = [x[0] for x in sorted_list]
                inv_sorted_list = [sorted_list2.index(i) for i in range(NUM_NEW_ENTRIES)]
            ...
            while len(new_entries) < NUM_NEW_ENTRIES:
                new_entries.append(FileType.MD.get_entry_model_cls().new_empty(u16_checked(len(new_entries))))
            ...
            while read_u16(rom.arm9, x) != 0:
                ...
                if new_entries[NUM_PREVIOUS_ENTRIES + pkmn_id].gender != Gender.INVALID.value:
                    ...
                    try:
                        for entry in mappa_model.floor_lists[dungeon.mappa_index][f].monsters:
                            if (
                                ...
                                and entry.md_index + NUM_PREVIOUS_ENTRIES < len(md_model.entries)
                                and md_model.entries[entry.md_index + NUM_PREVIOUS_ENTRIES].gender
