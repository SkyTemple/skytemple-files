...
class MdWriter:
    ...
    def write(self) -> bytes:
        ...
        for entry in self.model.entries:
            ...
            self._write_data(entry.base_hp, signed=True)
            self._write_data(entry.recruit_rate2, signed=True)
            self._write_data(entry.base_atk, 1, signed=True)
            self._write_data(entry.base_sp_atk, 1, signed=True)
            self._write_data(entry.base_def, 1, signed=True)
            self._write_data(entry.base_sp_def, 1, signed=True)
            ...
            self._write_data(entry.unk19, 1, signed=True)
            self._write_data(entry.unk20, 1, signed=True)
            self._write_data(entry.unk21, signed=True)
