...
OLD_ENTRIES: Dict[str, Dict[str, Tuple[bool, bool]]] = {
    ...
    "overlay/overlay_0010.bin": {
        "BadPoisonDamageDelay": (True, False),
...
class MigrationTestCase(unittest.TestCase):
    ...
    def test_backwards_compatibility(self, region):
        to_check = {
            x.filepath: x for x in load_binaries(f"{GAME_VERSION_EOS}_{region}")
        }
        ...
        for bin_path, symbol_specs in OLD_ENTRIES.items():
            ...
            for symbol_name, (has_end, is_function) in symbol_specs.items():
                self.assertIn(
                    symbol_name,
                    bin_symbols,
                    f"Symbol {symbol_name} in binary {bin_path} must be defined.",
                )
                ...
                if has_end:
                    try:
                        ...
                        self.fail(
                            f"Symbol {symbol_name} in binary {bin_path} must have a defined end."
                        )
                if is_function:
                    self.assertTrue(
                        symbol.type == Pmd2BinarySymbolType.FUNCTION,
                        f"Symbol {symbol_name} in binary {bin_path} must be a function.",
                    )
                else:
                    self.assertTrue(
                        symbol.type == Pmd2BinarySymbolType.DATA,
                        f"Symbol {symbol_name} in binary {bin_path} must be data.",
                    )
