...
class SpriteCollabOfflineTestCase(IsolatedAsyncioTestCase, ImageTestCaseAbc):
    ...
    async def test_fetch_portraits(self):
        async with self.client as session:
            result = await session.fetch_portraits(
                [
                    (9999, ""),
                    (9999, "9999/9999")
                ]
    ...
    async def test_fetch_sprites_unfiltered(self):
        async with self.client as session:
            ...
            self.assertSpritesEqual(
                os.path.join(FIXTURES_DIR, "sprite", "1"),
                *result[0]
            )
            self.assertSpritesEqual(
                os.path.join(FIXTURES_DIR, "sprite", "2"),
                *result[1]
            )
    ...
    async def test_fetch_sprites_filtered(self):
        async with self.client as session:
            ...
            assert result[1] is not None   # we do it like this for mypy
            self.assertSpritesEqual(
                os.path.join(EXPECTED_DIR, "sprite", "filtered1"),
                *result[0]
            )
            self.assertSpritesEqual(
                os.path.join(FIXTURES_DIR, "sprite", "2"),
    ...
    async def test_fetch_sprites_copy_event_sleep_no_target(self):
        async with self.client as session:
            ...
            assert result[0] is not None   # we do it like this for mypy
            self.assertSpritesEqual(
                os.path.join(EXPECTED_DIR, "sprite", "event_sleep1"),
                *result[0]
            )
    ...
    async def test_fetch_sprites_copy_event_sleep(self):
        async with self.client as session:
            ...
            assert result[0] is not None   # we do it like this for mypy
            self.assertSpritesEqual(
                os.path.join(EXPECTED_DIR, "sprite", "event_sleep2"),
                *result[0]
            )
    ...
    async def test_fetch_sprites_copy_event_sleep_partial(self):
        async with self.client as session:
            ...
            assert result[0] is not None   # we do it like this for mypy
            self.assertSpritesEqual(
                os.path.join(EXPECTED_DIR, "sprite", "event_sleep4"),
                *result[0]
            )
    ...
    async def test_fetch_sprites_copy_event_sleep_no_effect(self):
        ...
        async with self.client as session:
            ...
            assert result[0] is not None   # we do it like this for mypy
            self.assertSpritesEqual(
                os.path.join(EXPECTED_DIR, "sprite", "event_sleep3"),
                *result[0]
            )
    ...
    def assertSpritesEqual(self, expected_dir: str, sprite: Optional[WanFile], sprite_names: Pmd2Sprite):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            the_same = (
                len(diff_files) == 0 and
                len(cmp.funny_files) == 0 and
                len(cmp.left_only) == 0 and
            ...
            if not the_same:
                ...
                print("====================================")
                print("")
            self.assertTrue(the_same, "The exported sprite directories must be the same. Comparison report see above.")
