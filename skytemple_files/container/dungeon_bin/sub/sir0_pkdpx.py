...
class DbinSir0PkdpxHandler(DataHandler[CompressionContainerProtocol]):
    ...
    def deserialize(
        ...
        assert (
            len(sir0.content_pointer_offsets) == 0
        ), "The Sir0 contains an unexpected amount of pointers."
        # We don't support a data pointer, because we will also serialize without.
        assert (
            sir0.data_pointer == 0
        ), "The Sir0 contains a data pointer. That is not supported."
