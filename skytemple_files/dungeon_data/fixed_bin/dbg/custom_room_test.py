...
monster_stats_table = HardcodedFixedFloorTables.get_monster_spawn_stats_table(
    ov10, static_data
)
...
assert ov29_before != get_binary_from_rom_ppmdu(
...
assert monster_table == HardcodedFixedFloorTables.get_monster_spawn_list(
    ov29, static_data
)
assert tile_table == HardcodedFixedFloorTables.get_tile_spawn_list(ov29, static_data)
assert entity_table == HardcodedFixedFloorTables.get_entity_spawn_table(
    ov29, static_data
)
...
assert (
    get_binary_from_rom_ppmdu(
        NintendoDSRom.fromFile(os.path.join(output_dir, "test_fixed_floor.nds")),
        static_data.binaries["overlay/overlay_0029.bin"],
    )
    != ov29_before
)
