...
class BmaCollisionRleCompressionContainer(CommonAt):
    ...
    def decompress(self) -> bytes:
        ...
        data, len_read = FileType.BMA_COLLISION_RLE.decompress(self.compressed_data, self.length_decompressed)
        assert len_read == len(self.compressed_data)
        return data

    def to_bytes(self) -> bytes:
        return b"BMARLE" + self.length_decompressed.to_bytes(2, "little") + self.compressed_data  # pylint: disable=no-member
