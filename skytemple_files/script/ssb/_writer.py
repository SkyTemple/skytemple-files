...
from skytemple_files.common.ppmdu_config.data import GAME_REGION_US, GAME_REGION_EU, GAME_REGION_JP
from skytemple_files.common.ppmdu_config.script_data import Pmd2ScriptData
from skytemple_files.common.util import *
from skytemple_files.script.ssb._header import SSB_HEADER_US_LENGTH, SsbHeaderUs, SSB_HEADER_EU_LENGTH, SsbHeaderEu, \
    SSB_HEADER_JP_LENGTH, SsbHeaderJp, AbstractSsbHeader
from skytemple_files.script.ssb._model import Ssb
from skytemple_files.script.ssb import SSB_PADDING_BEFORE_ROUTINE_INFO
from skytemple_files.script.ssb.handler import SsbWriterProtocol

logger = logging.getLogger(__name__)


class SsbWriter(SsbWriterProtocol[Ssb]):
    """
    Writes a SSB model to a binary stream for saving in ROM.
    Also updates the original_binary_data field of the model.
    """
    def __init__(
            self, script_data: Pmd2ScriptData, 
    ...
    def write(self, model: Ssb) -> bytes:
        """
        ...
        elif self.game_region == GAME_REGION_EU:
            header_cls = SsbHeaderEu
            header_len = SSB_HEADER_EU_LENGTH
        elif self.game_region == GAME_REGION_JP:
            header_cls = SsbHeaderJp
            header_len = SSB_HEADER_JP_LENGTH
        else:
            raise ValueError(f"Unsupported game region {self.game_region}")
        ...
        data = bytearray(header_len + 6 * len(model.routine_info) + 2048)
        ...
        if len(model.strings) == 0:
            ...
            model.strings = {lang_name: [] for lang_name in header_cls.supported_langs()}
        number_of_strings = len(model.strings[next(iter(model.strings.keys()))])

        # Routine Info - The offsets used for the routine starts MUST already be correctly calculated!
        for offset, routine_info in model.routine_info:
        ...
        op_codes = self.script_data.op_codes__by_id
        for i, ops in enumerate(model.routine_ops):
            for op in ops:
                self.write_uintle(data, op.op_code.id, self.bytes_written, 2)
                self.bytes_written += 2
                if self.script_data.op_codes__by_id[op.op_code.id].params == -1:
        ...
        len_of_const_table = 2 * len(model.constants)
        ...
        if len({len(i) for i in model.strings.values()}) != 1:
        ...
        for lang in header_cls.supported_langs():
            ...
            for string in model.strings[lang]:
        ...
        if header_cls == SsbHeaderUs:
            # Number of constants
            write_uintle(data, len(model.constants), 0x00, 2)
            # Number of Strings
        ...
        write_uintle(data, len(model.routine_info), header_len + 2, 2)
