...
class AtupxTestCase(SkyTempleFilesTestCase[AtupxHandler, CompressionContainerProtocol]):
    @classmethod
    def handler(cls) -> Type[AtupxHandler]:
        return AtupxHandler

    def test_cross_999_native_implementation_compress(self):
    ...
    def test_cross_999_native_implementation_decompress(self):
        """Tests the native implementation against the Python implementation (assuming this one works)"""
        if not env_use_native():
            self.skipTest(
                "This test is only enabled when the native implementations are tested."
    ...
    def test_container(self, _, in_bytes):
        ...
        self.assertTrue(model.to_bytes().startswith(b"ATUPX"))
        self.assertEqual(
            model.to_bytes(),
