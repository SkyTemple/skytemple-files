...
from skytemple_files.dungeon_data.mappa_bin.protocol import MappaBinProtocol, MappaFloorLayoutProtocol, \
    MappaMonsterProtocol, MappaTrapListProtocol, MappaItemListProtocol
from skytemple_files_test.case import SkyTempleFilesTestCase, fixpath, romtest
from skytemple_files_test.dungeon_data.mappa_bin.fixture import eq_mappa_floor_layout_protocol, \
    eq_mappa_floor_terrain_settings_protocol, eq_mappa_item_list_protocol, eq_mappa_trap_list_protocol, \
    eq_mappa_monster_protocol, eq_mappa_floor_protocol, MappaFloorLayoutStub, MappaMonsterStub, MappaTrapListStub, \
    MappaItemListStub
from skytemple_files_test.dungeon_data.mappa_bin.fixture_autogen import (
    FIX_FLOOR_LAYOUTS, FIX_TERRAIN_SETTINGS_LISTS, FIX_MONSTER_LISTS, FIX_TRAP_LISTS,
    FIX_ITEM_LISTS, FIX_FLOOR_LISTS, FIX_FLOORS
)
...
class MdTestCase(SkyTempleFilesTestCase[MappaBinHandler, MappaBinProtocol]):
    ...
    def test_mappa_floor_layout_attrs(self):
        ...
        e.initial_enemy_density = u8(123)
        ...
        self.assertEqual(e.initial_enemy_density, u8(123))
    ...
    def test_mappa_monster_attrs(self):
        e = self.handler.get_monster_model()(
            u8(0), u16(0), u16(0), u16(0)
        )
    ...
    def test_mappa_trap_list__init__(self):
        for expected in FIX_TRAP_LISTS:
            subject = self.handler.get_trap_list_model()(
                expected.weights
            )
    ...
    def test_mappa_trap_list__eq__(self):
        subject_not_same = self.handler.get_trap_list_model()(
                FIX_TRAP_LISTS[0].weights
            )

        for expected in FIX_TRAP_LISTS[1:]:
            subject1 = self.handler.get_trap_list_model()(
                expected.weights
            )
            subject2 = self.handler.get_trap_list_model()(
                expected.weights
            )
    ...
    def test_mappa_trap_list_attrs(self):
        e = self.handler.get_trap_list_model()(
            [u16(0)] * 25
        )

        self.assertEqual(e.weights, {
            u8(0): u16(0),
            u8(1): u16(0),
            u8(2): u16(0),
            u8(3): u16(0),
    ...
    def test_mappa_floor_attrs(self):
        ...
        (e.layout, e.monsters, e.traps,
         e.floor_items, e.shop_items,
         e.monster_house_items, e.buried_items,
         e.unk_items1, e.unk_items2) = self._make_floor_model_params(
            FIX_FLOORS[1].layout,
    ...
    def test_mappa__init__(self):
        ...
        for in_list in FIX_FLOOR_LISTS:
            ...
            for in_floor in in_list:
                out_list.append(self.handler.get_floor_model()(
                    *self._make_floor_model_params(
                        in_floor.layout,
                        in_floor.monsters,
                        in_floor.traps,
    ...
    def test_mappa_add_floor_list(self):
        ...
        new_list = [self.handler.get_floor_model()(
            *self._make_floor_model_params(
                FIX_FLOORS[3].layout,
    ...
    def test_mappa_add_floor_to_floor_list(self):
        ...
        subject.add_floor_to_floor_list(0, self.handler.get_floor_model()(
            *self._make_floor_model_params(
                FIX_FLOORS[3].layout,
                FIX_FLOORS[3].monsters,
                FIX_FLOORS[3].traps,
        ...
        subject.add_floor_to_floor_list(1, self.handler.get_floor_model()(
            *self._make_floor_model_params(
                FIX_FLOORS[4].layout,
    ...
    def test_insert_floor_in_floor_list(self):
        ...
        subject.insert_floor_in_floor_list(0, 1, self.handler.get_floor_model()(
            *self._make_floor_model_params(
                FIX_FLOORS[3].layout,
                FIX_FLOORS[3].monsters,
                FIX_FLOORS[3].traps,
        ...
        subject.insert_floor_in_floor_list(1, 0, self.handler.get_floor_model()(
            *self._make_floor_model_params(
                FIX_FLOORS[4].layout,
                FIX_FLOORS[4].monsters,
                FIX_FLOORS[4].traps,
    ...
    def test_using_rom(self, _, file):
        ...
        after = self.handler.get_model_cls().sir0_unwrap(
            data, header_pointer
        )
    ...
    def _make_floor_model_params(
        ...
        return (
            ...
            self.handler.get_trap_list_model()(
                traps.weights
            ),
            self.handler.get_item_list_model()(
                floor_items.categories,
                floor_items.items
            ),
            self.handler.get_item_list_model()(
                shop_items.categories,
                shop_items.items
            ),
            self.handler.get_item_list_model()(
                monster_house_items.categories,
                monster_house_items.items
            ),
            self.handler.get_item_list_model()(
                buried_items.categories,
                buried_items.items
            ),
            self.handler.get_item_list_model()(
                unk_items1.categories,
                unk_items1.items
            ),
            self.handler.get_item_list_model()(
                unk_items2.categories,
                unk_items2.items
            ),
        )
