...
class Pmd2BinarySymbol(AutoString):
    def __init__(
            self, name: str, begin: int, end: Optional[int],
            description: str, typ: Optional[Pmd2BinarySymbolType] = None
    ):
        self.name: str = name
        ...
        self.type: Pmd2BinarySymbolType = typ if typ is not None else Pmd2BinarySymbolType.DATA
    ...
    def begin_absolute(self) -> int:
        if self.parent is None:
            raise RuntimeError("Absolute begin of this symbol is unknown (no parent assigned).")
...
class Pmd2Binary(AutoString):
    def __init__(self, filepath: str, loadaddress: int, length: int, description: str, symbols: List[Pmd2BinarySymbol]):
        self.filepath = filepath
        self.loadaddress = loadaddress
        self.length = length
        self.description = description
        self.symbols: MultiCasingDict[Pmd2BinarySymbol] = MultiCasingDict((x.name, x) for x in symbols)
    ...
    def functions(self) -> Dict[str, Pmd2BinarySymbol]:
        """For backwards compatibility."""
        warnings.warn(_DEPRECATION_WARNING, DeprecationWarning)
        return {k: v for k, v in self.symbols.items() if v.type == Pmd2BinarySymbolType.FUNCTION}
