...
class RomFileProviderStub(RomFileProviderProtocol):
    def getFileByName(self, filename: str) -> bytes:
        ...
        if filename == "MAP_BG/coco.bma":
            with open(os.path.join(dirn, "..", "fixtures", "MAP_BG", "coco.bma"), "rb") as f:
                return f.read()
        if filename == "MAP_BG/coco.bpc":
            with open(os.path.join(dirn, "..", "fixtures", "MAP_BG", "coco.bpc"), "rb") as f:
                return f.read()
        if filename == "MAP_BG/coco.bpl":
            with open(os.path.join(dirn, "..", "fixtures", "MAP_BG", "coco.bpl"), "rb") as f:
                return f.read()
        if filename == "MAP_BG/coco1.bpa":
            with open(os.path.join(dirn, "..", "fixtures", "MAP_BG", "coco1.bpa"), "rb") as f:
                return f.read()
