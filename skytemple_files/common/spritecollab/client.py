...
class MonsterFormDetails(MonsterFormInfo):
    ...
    async def fetch_portrait_via_face_name(self, face_name: Pmd2ScriptFaceName) -> Optional[Image.Image]:
...
class SpriteCollabSession:
    ...
    def __init__(self, session: AsyncClientSession, ds: DSLSchema, request_adapter: AioRequestAdapter):
    ...
    async def fetch_config(self) -> Config:
        ...
        return (await self.execute_query(DSLQuery(
    ...
    async def list_monster_forms(self, with_preview_portrait: bool):
        ...
        if with_preview_portrait:
            portrait_extra_args.append(
                self.ds.MonsterFormPortraits.previewEmotion.select(
                    self.ds.Portrait.url
                )
            )

        result = await self.execute_query(DSLQuery(
        ...
        return await self._fetch_details(
            ...
            lambda credit, sprite, copy_of: (
                ...
                self.ds.MonsterForm.sprites.select(
                    self.ds.MonsterFormSprites.phase,
                    self.ds.MonsterFormSprites.modifiedDate,
                    self.ds.MonsterFormSprites.actions.select(
                        sprite, copy_of
                    ),
    ...
    async def fetch_portraits(
        ...
        Raises an error if any path could not be resolved to a valid form.
        """
        async def process_form(_idx: int, _monster: Monster_Metadata, form: MonsterForm) -> List[Optional[KaoImageProtocol]]:
    ...
    async def fetch_sprites(
        ...
        async def process_form(idx: int, monster: Monster_Metadata, form: MonsterForm) -> Optional[Tuple[WanFile, Pmd2Sprite]]:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                try:
                    ...
                    raise RuntimeError(f"Error processing AnimData.xml for sprite for form of monster {monster['id']}.") from ex

                return (
                    CharaWanHandler.import_sheets(tmp_dir), Pmd2Sprite(-1, action_indices)
                )
    ...
    async def execute_query(self, query: DSLQuery, *, fragments: Optional[Iterable[DSLFragment]] = None) -> Query:
    ...
    async def _fetch_details(
            ...
            selector_fn: Callable[[DSLFragment, DSLFragment, DSLFragment], Iterable[DSLField]],
    ...
    async def _fetch_details_single_mon(
        ...
        for path in paths:
            key = f"f_{i}"
            monster_forms[key] = self.ds.Monster.manual(path=path).select(*selector_fn(credit, sprite, copy_of))
    ...
    async def _fetch_details_multi_mons(
        ...
        result = await self.execute_query(DSLQuery(
            self.ds.Query.monster(filter=monster_ids).select(
                self.ds.Monster.id,
                self.ds.Monster.name,
                self.ds.Monster.forms().select(*selector_fn(credit, sprite, copy_of))
            )
        ), fragments=fragments)
...
class SpriteCollabClient:
    ...
    def __init__(
        ...
        if isinstance(transport_or_schema, AsyncTransport):
            self._client = Client(transport=transport_or_schema, fetch_schema_from_transport=True)
        else:
